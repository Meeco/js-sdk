import * as sdk from '@meeco/sdk';
import { DelegationService, SymmetricKey, UserService } from '@meeco/sdk';
import { expect } from '@oclif/test';
import { readFileSync } from 'fs';
import { customTest, outputFixture, testEnvironmentFile, testUserAuth } from '../../test-helpers';

describe('delegations:load-auth-config', () => {
  customTest
    .stub(sdk.mockableFactories, 'keystoreAPIFactory', keystoreAPIFactory as any)
    .stub(sdk.mockableFactories, 'vaultAPIFactory', vaultAPIFactory as any)
    .stdout()
    .stderr()
    .stub(DelegationService.prototype, 'getAccountOwnerKek', getAccountOwnerKek as any)
    .stub(UserService.prototype, 'getDataEncryptionKey', getDataEncryptionKey as any)
    .run([
      'delegations:load-auth-config',
      ...testUserAuth,
      ...testEnvironmentFile,
      '--delegationId',
      '223009b5-bd8f-454b-b117-33f553e5ee06',
    ])
    .it('creates a delegation connection invitation', ctx => {
      const expected = readFileSync(
        outputFixture('delegations-load-auth-config.output.yaml'),
        'utf-8'
      );
      expect(ctx.stdout).to.contain(expected.trim());
    });
});

function keystoreAPIFactory(environment) {
  return authConfig => ({
    DelegationApi: {
      delegationsDelegationTokenGet: delegationToken =>
        Promise.resolve({
          delegation: {
            delegation_token: '309963f7-a103-496b-a136-ce78b9fbcbd0',
            claimed_at: '2021-07-13T06:51:15.628Z',
            expire_at: '2022-01-11T21:45:29.864Z',
            verification_attempted_at: '2021-07-13T07:23:03.570Z',
            verified: true,
            delegation_role: 'reader',
            delegate_public_key:
              '-----BEGIN PUBLIC KEY-----\r\\r\n-----END PUBLIC KEY-----\r\n',
            delegate_signature:
              'Sign.Rsa4096.bVGh6yAMo_bjFF3qhSJUOUUwOb8QUPqcXiq5vy2AsWB1p4aAI99qXOz9YXxOR51USYmJUbsstgJ0t8ZXB9yZBMLW_ile2w6khylsFxb91w1WwGb1Jy7PZLjnETxFgeMZV4Vfa_Y-lZVlTKm7nhye-2UYWcoUb7rTjXk_hSMQRGkCVnuwQIAodCo2sDxYmDdbGS03euf7H_I0TklLNEb_vqp-Fb-JIbTPJ72cCs9tP7my670u_MaS_zRvSRFP04rxAqRXr0dqoALnNrJi3A35JDtPcQoNP7It4QDm6w_mybpK0U6eK0HjgetQlMB8If1TcKdRE5I3yB9MzMIqw39ls9PHIZU18NnaZi77ROrOMN8M0lLuVxRTxwfKG0uWHalTeEXa_0sXMGW48b-PMN_yphA7NEuDUUemYTvQJFCxQ2TdV2jOxbNz-EyLlh6OduEYYfEnrKgqaCvISiD5A495Zibhi_jXKc8qiRrJoqtNFFJpDhSGxmsyw1zv8C4hwtn-btBD310Y4gvmLcPjSDh2ZI-8EC0ixnjJVIPe7r6fZQzKZvAnKN98SoJI2fdXTZEP7k-XqT6MQNk0KkKQVuuWzhyUbSYXpWnsy4Qxy07QDUSv5kYgie9EdR3cMzu1T1mqa4Dr0NQ69pStCKYaSiE2YVV7R4xrO5vjIKqcKNz1yzo=.MzA5OTYzZjctYTEwMy00OTZiLWExMzYtY2U3OGI5ZmJjYmQw',
            vault_account_owner_id: '223009b5-bd8f-454b-b117-33f553e5ee06',
            account_owner_kek_encrypted_with_connection_keypair: false,
            account_owner_kek:
              'Aes256Gcm.j6rAPNThvcHJkR5Zcua9C2dgdf43d5hzzDwc75t9w6k=.QUAAAAAFaXYADAAAAADZcm8_T7q3eYFG5gUFYXQAEAAAAAAVwqDpPngfYu-P0ilAl9uDAmFkAAUAAABub25lAAA=',
          },
        }),
    },
  });
}

function vaultAPIFactory(environment) {
  return authConfig => ({
    ConnectionApi: {
      connectionsGet: cursor =>
        Promise.resolve({
          next_page_after: null,
          connections: [
            {
              own: {
                id: 'a2b78a17-8033-4afc-bcbe-728a4386fbc4',
                encrypted_recipient_name:
                  'Aes256Gcm.LIIzwfk=.QUAAAAAFaXYADAAAAAAS-2WPy4OLYGoP8S8FYXQAEAAAAAA4OODPFr4IExFBJ6P0-qTPAmFkAAUAAABub25lAAA=',
                integration_data: null,
                connection_type: null,
                user_id: '7b2a5f76-da09-4395-8a0f-9d5d0801faae',
                user_type: 'human',
                user_public_key:
                  '-----BEGIN PUBLIC KEY-----\r\\r\n-----END PUBLIC KEY-----\r\n',
                user_keypair_external_id: '02826915-966a-4001-bab6-e53d03471f6c',
              },
              the_other_user: {
                id: '2322b5ed-7933-4ce4-9e1c-83f65eff79aa',
                integration_data: {
                  role: 'reader',
                  intent: 'delegate',
                  delegation_token: '309963f7-a103-496b-a136-ce78b9fbcbd0',
                },
                connection_type: 'delegate',
                user_id: '223009b5-bd8f-454b-b117-33f553e5ee06',
                user_type: 'human',
                user_public_key:
                  '-----BEGIN PUBLIC KEY-----\r\\r\n-----END PUBLIC KEY-----\r\n',
                user_keypair_external_id: 'dcd172a1-588e-4a1b-8346-d328e8caf9cf',
              },
            },
          ],
        }),
    },
    UserApi: {
      meGet: cursor =>
        Promise.resolve({
          user: {
            id: '223009b5-bd8f-454b-b117-33f553e5ee06',
            full_name: null,
            email: '',
            country: null,
            joined_at: null,
            onboarded_at: null,
            track_events: true,
            track_usage: true,
            timezone: null,
            private_dek_external_id: '0bc362b7-93c9-4c99-b259-0a96b688b5b0',
            queued_for_deletion_after: null,
            accepted_terms: false,
          },
        }),
    },
  });
}

function getAccountOwnerKek(authConfig, delegation) {
  // return a SymmetricKey (with encryptionkey inside)
  return Promise.resolve(
    SymmetricKey.fromSerialized('GV5zf72jRXwQJNa6rLQwxGzn7KoU3xQBePnrL3dYMcY=')
  );
}

function getDataEncryptionKey(authConfig, dekId) {
  // return a SymmetricKey (with encryptionkey inside)
  return Promise.resolve(
    SymmetricKey.fromSerialized('hZHHXNXVDOZEhwbZZHC82A4I98LaFjPUSNj-prBEgEs=')
  );
}
