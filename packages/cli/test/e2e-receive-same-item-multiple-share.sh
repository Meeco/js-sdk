...
run shares:create-config --from .Alice.yaml --connectionId $connectionIdAC -i $itemId > .share_Alice_Carol.yaml
run shares:create -c .share_Alice_Carol.yaml -m anyone -d $dateAFter30Days > .share_Alice_Carol.created.yaml
